...
//-------------------------------------------------------------------------
    fn reset(env: &mut I2cEnv, i2c_adrs: u8) {
        ...
        unsafe{delay_msec(15);}
        env.write_dt(i2c_adrs, &i2c_data);
        unsafe{delay_msec(900);}
    }
    //-------------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------------
    pub fn change_sensitivity(env: &mut I2cEnv, data: u8, number: usize) {		//	data : 0-3
    ...
    //-------------------------------------------------------------------------
    fn check_write_config(env: &mut I2cEnv, check_sum_l: u8, check_sum_h: u8, crnt_adrs: u8) -> i32 {
        let rd_dt = env.read_dt::<2>(crnt_adrs, &[CONFIG_CRC]);
        if ( rd_dt[0] == check_sum_l ) && ( rd_dt[1] == check_sum_h ){ return 0; }
    ...
    fn write_config(env: &mut I2cEnv, number: usize, i2c_adrs: u8) -> i32 {
        ...
        //	Compare both Data
        for i in 0..CONFIG_DATA_SZ {
            if config_data[i] != rd_dt[i] {return i as i32;}
        }
        0
    }
}
